...
class Academy(http.Controller):
    ...
    # added by abrar
    @http.route('/channel', auth='public', csrf=False, website=True)
    def index(self, **kw):
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # return "controller is calling"
        # return request.render('academy.news_created',{})
        return request.render('academy.user_details', {})
    ...
    # Added by Appscomp
    @http.route('/info', auth='public', csrf=False, website=True)
    def list(self, **kw):
        return request.render('academy.city_details', {
            'objects': request.env['res.partner'].search([]),
        })
    # Ended by Appscomp

    @http.route('/academy/<int:id>/', csrf=False, auth='public', website=True)
